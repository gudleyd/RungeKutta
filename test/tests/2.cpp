...
static int get_flag(int mask, size_t i) {
    return ((mask & (1 << i)) != 0);
}
...
int mass_test_2(std::vector<long double> (*solver)(const rk::Expression<long double>&, std::vector<long double>, long double, long double),
    ...
    {   /*  SOLVE TESTS */
        ...
        /*
        {
            ...
            tmpErrCount += sTest0.run_solve_test({0, 1}, precision, logFile, solver);
        }
        */
        
        if (get_flag(bitMask, 0)) {
            // y = e^(2x) [long doubles]
            tests_rk::BasicTest<long double> sTest1("2 * y", {"x", "y"}, delta, {{0.0, 1}, {1, 7.38906}, {2, 54.59815}, {0.5, 2.71828}, {1.5, 20.08554}}, 
            {2, 14.77812, 0.27067, 109.1963, 0.0366312, 5.43656, 0.735758});
            tmpErrCount += sTest1.run_solve_test({0, 1}, precision, logFile, solver);
        }

        if (get_flag(bitMask, 1)) {
            // y = sin(x) * e^(2x) + 1
            ...
            tmpErrCount += sTest2.run_solve_test({0, 1}, precision, logFile, solver);
        }

        if (get_flag(bitMask, 2)) {
            // y =  e^(sin(x)^2) - 1
            ...
            tmpErrCount += sTest3.run_solve_test({0.1, 0.0100165}, precision, logFile, solver);
        }
        
        if (get_flag(bitMask, 3)){
            // y =  e^e^e^x
            ...
            tmpErrCount += sTest4.run_solve_test({-1, 4.24044349228}, precision, logFile, solver);
        }
        if (get_flag(bitMask, 4)) {
            // y =  e^( -e^(sin(x) - cos(x)) )
            ...
            tmpErrCount += sTest5.run_solve_test({-1, 1.77791903645}, precision, logFile, solver);
        }
        if (get_flag(bitMask, 5)){
            // y =  sqrt(x) +1/x
            ...
            tmpErrCount += sTest6.run_solve_test({0.1, 10.316227766}, precision, logFile, solver);
        }
        if (get_flag(bitMask, 6)) {
            // y =  x + 5
            ...
            {1, 1, 1, 1, 1, 1, 1, 1});
            tmpErrCount += sTest7.run_solve_test({-5, 0}, precision, logFile, solver);
        }
        if (get_flag(bitMask, 7)) {
            // y =  x^2
            ...
            {-2, -1, 0, 1, 2, 4, 10});
            tmpErrCount += sTest8.run_solve_test({-5, 25}, precision, logFile, solver);
        }
        if (get_flag(bitMask, 8)) {
            // y = 100
            ...
            {0, 0, 0, 0, 0, 0, 0});
            tmpErrCount += sTest8.run_solve_test({-100, 100}, precision, logFile, solver);
        }
        if (get_flag(bitMask, 9)) {
            // y = e^(-sin^6(x)) - 1/2(1 + e^(-1))

            tests_rk::BasicTest<long double> sTest8("-6 * (y - 0.683939720586) * pow(sin(x), 5) * cos(x)", 
            {"x", "y"}, delta, 
            {{-0.5, 0.303990680668}, {-0.25, 0.315830986794}, {0, 0.316060279414}}, 
            {0.131756305789, 0.00538727827732, 0});
            tmpErrCount += sTest8.run_solve_test({-1, 0.017229985896}, precision, logFile, solver);
        }
        if (tmpErrCount > 0)
    ...
    out << "\nFinished running mass test " + name + "\n\n";
    out << std::string(64, '-') << '\n';
    return errCount;
}