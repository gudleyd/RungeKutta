...
int mass_test_2(std::ostream& out, std::ostream& logFile) {
    ...
    {   /*  SOLVE TESTS */
        ...
        out << "\nRunning solve tests...\n";
        {
            ...
            tmpErrCount += sTest0.run_solve_test({0, 1}, 0.00001, logFile);
        }
        {
            // y = e^(2x) [long doubles]
            tests_rk::BasicTest<long double> sTest1("2 * y", {"x", "y"}, 0.00001, {{0, 1}, {1, 7.38906}, {2, 54.59815}, {0.5, 2.71828}, {1.5, 20.08554}}, 
            {2, 14.77812, 0.27067, 109.1963, 0.0366312, 5.43656, 0.735758});
            tmpErrCount += sTest1.run_solve_test({0, 1}, 0.00001, logFile);
        }
        {
            // y = sin(x) * e^(2x) + 1
            tests_rk::BasicTest<long double> sTest2("2 * y + ( 1 - (2 * sin(0.5 * x) * sin(0.5 * x)) ) * y * (1.0 / sin(x))", {"x", "y"}, 0.00001, 
            ...
            tmpErrCount += sTest2.run_solve_test({0, 1}, 0.00001, logFile);
        }
        {
            // y =  e^(sin(x)^2) - 1
            ...
            tests_rk::BasicTest<long double> sTest3("sin(2 * x) * (y + 1)", {"x", "y"}, 0.00001, 
            ...
            tmpErrCount += sTest3.run_solve_test({0.1, 0.0100165}, 0.00001, logFile);
        }
        
        {
            ...
            tmpErrCount += sTest4.run_solve_test({-1, 4.24044349228}, 0.00001, logFile);
        }
        {
            // y =  e^( -e^(sin(x) - cos(x)) )
            ...
            tmpErrCount += sTest5.run_solve_test({-1, 1.77791903645}, 0.00001, logFile);
        }
        {
            // y =  sqrt(x) +1/x
            ...
            tmpErrCount += sTest6.run_solve_test({0.1, 10.316227766}, 0.00001, logFile);
        }
        {
            // y =  x + 5
            ...
            {1, 1, 1, 1, 1, 1, 1, 1});
            tmpErrCount += sTest7.run_solve_test({-5, 0}, 0.00001, logFile);
        }
        {
            // y =  x^2
            ...
            {-2, -1, 0, 1, 2, 4, 10});
            tmpErrCount += sTest8.run_solve_test({-5, 25}, 0.00001, logFile);
        }
        {
            // y = 100
            ...
            {0, 0, 0, 0, 0, 0, 0});
            tmpErrCount += sTest8.run_solve_test({-100, 100}, 0.00001, logFile);
        }
        if (tmpErrCount > 0)
